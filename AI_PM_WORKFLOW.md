...
- 主 `README.md` 的待辦只放「需要被解決的事情」，且必須有明確日期、期限或待確認日期；沒有時間要求的想法放到研究文件、設計文件或 `AI_PM_INBOX.md`。
- 每個待辦都要寫清楚對應目標與消光條件，並依截止時間排序。完成後更新狀態或移出待辦，不能讓已解決事項長期卡在清單裡。
- 每次刷新待辦時，都要用當天日期檢查：已過日期不留在主 README 待辦；若該活動或交付物有可記錄內容，補進對應日期的 `AI_PM_INBOX.md` 或正式日誌。
- 待辦不能發散成雜項清單；小事可以記錄，但正式追蹤時要連回具體目標，例如科會報告、雙週進度表、最終成果展示、評分表匯出或重要公司活動。
- 主 `README.md` 的重要交付物只放真的要提交、展示、匯出或供主管評核使用的成果。內部支援文件、AI 執行軌跡、一般 README、草稿素材可留在各自資料夾，不列為第一層重要交付物。
- 若日期未確定，要明確標示 `待確認`，並把「確認日期」本身列為待辦；不可自行猜測 HR、學校或公司正式繳交日。
