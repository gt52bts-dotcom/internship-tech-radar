...
- At the start of each task, read `PROJECT_MEMORY.md` and the most recent `logs/daily/work-log-YYYY-MM-DD.md` before making project decisions.
...
- On weekdays, do not create or finalize the current day's `logs/daily/work-log-YYYY-MM-DD.md` before 17:00 Asia/Taipei. During the day, append evidence to `AI_PM_INBOX.md`; the scheduled 17:00 run synthesizes the formal log.
...
- The weekday 17:00 synthesis must create or update the same daily log in both Git and the Notion journal database, then update the Notion Skill score database, GitHub/HTML interactive dashboard, and embedded Notion dashboard.
...
- After substantive project work before the weekday 17:00 synthesis, update `AI_PM_INBOX.md` instead of the current formal log. After 17:00, merge related actions into clear outcome-based language in `logs/daily/work-log-YYYY-MM-DD.md`.
