...
## 2026-08-03 Mentor 收斂方向

- S5 報告的一句摘要改為「新聞摘要：應用面優勢」，重點放在新功能對實際應用面帶來哪些優勢，而不是只寫 PoC 是否成功。
- S1-S5 每一步都要顯示證據；分數要標滿分，不在人類摘要寫信心。
- 報價單要讓人能判斷價值：先寫預期情境假設，再列明細；指出人類需確認的資源、最貴項目，以及什麼用量或時間會使費用增加。Lambda 要說明只有被呼叫時才依 request 與執行時間計費。
- 最後兩週進入收斂期：final proposal / 論文素材要回答「這篇新聞在 PoC 裡還值得做什麼」與「reviewer 會問什麼」，並保留 Future work，不再無限制擴功能。
...
- 建立單一入口的端到端評估流程。
- S1-S2 可掃描與比較多個候選；S3 起由人類選定單一候選進入 Evaluate、Validate、Report。
- 評分、技術依據、成本與選擇結果可追溯。
- 能整理成公司帳戶可部署、可交接的封包。
...
flowchart TD
    ...
    C --> D["S1 掃描"] --> E["S2 比較"] --> F["人類選定單一候選"]
    F --> G["S3 評估<br/>滿分 5 與報價"] --> H["S4 受控 PoC 驗證"] --> I["S5 證據報告<br/>Future work"]
...
- S1 掃描來源與候選。
- S2 比較候選證據、Region、價格與待確認問題，不自動決定勝出者。
- 人類選定單一候選後進入 S3。
- S3 依固定 rubric 評估滿分 5 分，並產出 PoC 預估報價單。
- S4 在具名授權、成本上限、recipe、Console review 與 cleanup gate 下進行受控 PoC 驗證。
- S5 產出技術驗證報告，顯示 S1-S5 證據、成本狀態、reviewer questions 與 Future work。
...
| 成效面向 | 目前證據 | 簡報標示 |
|---|---|---|
| 流程完整性 | S1-S2 多候選掃描與比較；S3-S5 單一人類選定候選完整證據鏈 | 已實作並以兩條 AWS 官方文章 PoC 驗證 |
...
- [ ] 用新版單一候選流程整理一個完整成功案例的輸入、評估過程、報價、S4 驗證、S5 報告、reviewer questions 與 Future work。
