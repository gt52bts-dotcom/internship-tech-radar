...
- cleanup verified runtime artifact

## GitHub 位置

- Skill 規格：`radar-redesign/skills/validate-cloud-poc/SKILL.md`
- Agent 入口：`radar-redesign/skills/validate-cloud-poc/agents/openai.yaml`
- 共用核心：`radar-redesign/agentic_cloud_radar/s4.py`
- Recipe registry：`radar-redesign/agentic_cloud_radar/s4_recipes/registry.py`

## 20 秒講稿

> Skill 4 不是展示 AI 會部署，而是證明這個 PoC 在受控條件下真的能建立、驗證、盤點和清除。它最大的亮點是部署前就有預算、recipe 和人類核准，部署後留下可回查證據，而不是只交一張成功截圖。
