...
- 不補未知事實。
- 不操作 AWS。
- 不做 AWS Billing / Cost Explorer / CUR 對帳。
- 不把 cleanup 前 runtime facts 換算成實際費用。
...
- Skill 規格：`radar-redesign/skills/report-cloud-evidence/SKILL.md`
- Agent 入口：`radar-redesign/skills/report-cloud-evidence/agents/openai.yaml`
- 共用核心：`radar-redesign/agentic_cloud_radar/s5.py`

## 20 秒講稿

> Skill 5 的價值是把整條 S1-S5 證據鏈翻成主管能讀的報告。它不補故事、不重新評分、不把估算講成帳單，而是清楚標示哪些已驗證、哪些只是預估、哪些還需要公司環境確認，讓 PoC 結果可以被交接和追問。
