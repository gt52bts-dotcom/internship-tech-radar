...
- For human-facing Skill 3 reports, Cleo prefers a GPT-style raster architecture infographic similar to AWS solution diagrams, not only Mermaid. In Agent mode, Codex may use the built-in image generation tool to create a reviewable PNG before S4 approval; keep Mermaid/text diagrams as a traceable fallback. Generated diagrams require human QA for text accuracy and must not be treated as evidence of deployed resources.
