...
- 近期固定時程：2026-08-03 10:30 部會出席聆聽，不需報告；2026-08-11 15:30 科會報告 AI PM；2026-07-30（四）上午人壽高管交流活動（總公司）；2026-08-06 至 2026-08-07 到信義區參加集團 AI 競賽，當日不進內湖辦公室；2026-08-10（一）人壽 1st 共融活動（六度空間）；2026-08-17（一）部會展示最終實習成果報告；2026-08-20（四）人壽 2nd 共融活動（總公司）；2026-08-28（五）13:30 國立臺灣海洋大學教授到公司訪視評分；2026-08-31（一）集團結訓典禮（國泰金融會議中心）。已過日期不可留在 README 待辦，活動後再補進當日 inbox / 正式日誌。
...
- 2026-07-28 使用者新增學校端正式訪視時程：2026-08-28（五）13:30 海大教授到公司訪視評分。使用者修正：訪視時不另外準備專案簡報重點或成果證據包，直接拿 2026-08-17 部會成果簡報／剪報給教授看；此日期同時也是 8/17-8/28 CIP 雙週工作進度整理日，需提前安排避免撞期。
...
- For human-facing Skill 3 reports, Cleo prefers a GPT-style raster architecture infographic similar to AWS solution diagrams. In Agent mode, Codex may use the built-in image generation tool to create a reviewable PNG before S4 approval; put that PNG directly in the report, not merely as a link, and do not also include the old Mermaid/text flowchart in the human-facing report. Use a same-folder relative `<img src="./...png">` or an explicitly embedded image form for the report preview; do not use `C:/Users/...` absolute local paths because some Markdown viewers render those as plain links. Mermaid/text diagrams may remain only as internal traceable fallback. Generated diagrams require human QA for text accuracy and must not be treated as evidence of deployed resources.
- If a Markdown preview still renders local/relative image references as URLs, produce a self-contained HTML report and/or data-URI image embed so the architecture PNG displays directly. For human review, prefer the HTML preview when the Markdown renderer blocks local image loading.
