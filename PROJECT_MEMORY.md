...
- 每日日誌需指出工作屬於五個 Skill 的哪一部分、依證據給予 `+5／+10／+15` 積分，並判斷能否扣回原始專案目標。

## 五個 Skill 儀表板

- 目前依技術雷達既有五步驟定義：S1 技術來源掃描、S2 公司情境排序、S3 AI 評估、S4 獨立驗證、S5 Top 3 與報告呈現；若 Mentor 另有正式命名，再整體替換。
- 積分規則：小進展 `+5`；完成可用成果 `+10`；完成且有驗證證據的里程碑 `+15`。
- 工作與目標的關係分成：`直接扣回目標`、`間接支援`、`偏離目標`。偏離目標可保留紀錄，但不計入 Skill 積分。
- `SKILL_PROGRESS.md` 保存每日與累積分數；README 顯示主管可快速閱讀的摘要儀表板。
