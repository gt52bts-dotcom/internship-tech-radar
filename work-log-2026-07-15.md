...
|---|---|---:|---|---|
| 檢查手動部署包、Lambda 套件與狀態機定義 | 掃描 | +3 | 直接扣回目標 | 離線檢查部署包內容與語法 |
| 對照公司限制調整部署方式與 IAM policy | 比較 | +3 | 直接扣回目標 | 從 CDK／自動部署轉為 Console 手動部署，並拆分 S3 權限 |
| 釐清 IAM policy、SCP 與資源權限的影響 | 評估 | +3 | 直接扣回目標 | 判斷 legacy parsing 與 Organizations SCP 的不同層級限制 |
| 驗證公司帳戶前置條件並追蹤第 1 至第 4 章部署狀態 | 驗證 | +6 | 直接扣回目標 | 公司帳戶 CLI 檢查、Console 操作回報、DynamoDB schema 記錄 |
| 建立 GitHub／Notion／日誌的主管閱讀與積分追蹤機制 | 報告 | +8 | 直接扣回目標 | GitHub private 儀表板、Notion 欄位與本日正式工作日誌 |
