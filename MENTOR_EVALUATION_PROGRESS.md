...
更新日期：2026-07-21  
用途：依實習主管評分表項目，從每日工作日誌、AI PM inbox、Git 產出與驗證證據中追蹤可支持的自評依據。正式成績仍以主管評分為準；無法從證據合理判斷的項目先留白。
...
這張表屬於 mentor 最後一週的行為觀察，不適合只靠 Git 產出或 AI 推論打分。2026-07-21 起，15 項先標為 `暫不評分`、不計平均；等使用者或 mentor 提供實際觀察後再補。

完整題目與待補資訊：`evaluation-forms/cathay-mentor-observation-form.md`

## 目前建議填表分數

- 四大項目目前平均：`4.5 / 5`。這只適用於上方四大項目自評。
- Mentor 15 項行為觀察：`暫不評分`、不計平均；需要主管／mentor 或使用者補實際觀察。
