...
S0 不應該直接負責向外搜尋資料。S0 的責任是把問題定義清楚，而不是找解法。

建議分層：

| 層級 | 是否需要 LLM | 是否需要外部搜尋 | 原因 |
|---|---|---|---|
| S-1 Problem Discovery | 可選 | 可選但限低敏感公開來源 | 用來產生候選問題與訪談提問，不宣稱公司真實痛點 |
| S0 Demand Input | 可選 | 不建議 | 協助整理需求卡、追問缺漏、檢查敏感資訊；避免還沒確認需求就外搜 |
| S1 Scan | 建議可用 | 是 | 在 S0 經人類確認後，依掃描邊界搜尋新技術與官方資料 |
| S2-S5 | 可選 | 依需要 | 協助比較、報價、評估與報告，但要保留來源與證據 |

所以 S0 可以有一個「需求整理助理」，但它不需要直接拿搜尋用 API key。真正需要 API key 的地方會是 S1 之後，例如：

- LLM API key：把搜尋結果摘要、分類、比較。
- Search API / RSS / 官方文件來源：蒐集候選技術。
- AWS API：做 read-only 驗證或後續 PoC。

安全規則：

- API key 只能放後端環境變數或 AWS Secrets Manager。
- GUI 不顯示、不輸入、不保存 API key。
- repo、日誌、報告都不能寫入 key。
- S0 需求卡若出現客戶資料、內部 URL、帳號、錯誤截圖中的敏感資訊，應先提示遮罩或拒絕送出。
- S1 外部搜尋只使用 S0 通過後的去識別化需求摘要。
