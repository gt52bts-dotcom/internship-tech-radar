...
| 日期 | 今日主軸 | 狀態 |
|---|---|---|
| [7/27](./logs/daily/work-log-2026-07-27.md) | 完成 AI PM 科會材料，並以真實 AWS 官方 URL 驗證新版雷達 S0→S1 本機鏈路 | direct |
...
2026-07-31（五）改為第一版完整交付硬截止：不是只整理雙週進度，而是要交出可給 Mentor 確認的完整 Skills 版本。接下來採短衝刺：先把 S0-S5 快速打通，再把五個 Skills 的輸入 / 輸出、跑法、檢測證據與限制整理成可審查 package。

| 日期 | 主軸 | 當日完成條件 |
|---|---|---|
| 2026-07-24（五） | 研究 S0 需求輸入層與 AI PM 科會內容。 | 已進行但未完成：S0 尚未讀完，CLI、unittest、compileall 不能列為完成證據。 |
| 2026-07-27（一） | 優先完成 AI PM 科會簡報，並收 S0 / S1 最小可跑切片。 | 科會簡報、10 分鐘講稿與 2-3 組證據先收斂；S0 CLI / unittest / compileall 有證據；S1 可從 confirmed demand card 產出可追溯掃描結果。 |
| 2026-07-28（二）下午 | 完成 S2 / S3 最小可跑切片。 | S2 可比較候選並保留依據；S3 可依 rubric 產出分數、風險、限制與 PoC gate 判斷。 |
...
| 海大教授公司訪視評分 | 2026-08-28（五）13:30 | 已排入時程。 | 訪視前完成評分資料、專案展示重點與必要佐證整理；訪視後補進當日紀錄。 |
...
| 2026-08-28（五）13:30 | 海大教授到公司訪視評分。 | 學校端正式訪視與評分。 | 訪視前準備專案簡報重點、成果證據、評分表與可回答的限制 / 下一步；訪視後補進當日 inbox / 日誌。 | 未開始 |
