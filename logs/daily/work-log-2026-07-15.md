...
|---|---|---:|---|---|
| 檢查手動部署包、Lambda 套件與狀態機定義 | 掃描 | +1 | 直接扣回目標 | 離線檢查部署包內容與語法；未在公司帳戶執行 |
| 對照公司限制調整部署方式與 IAM policy | 比較 | +1 | 直接扣回目標 | 從 CDK／自動部署轉為 Console 手動部署，屬部署策略調整 |
| 釐清 IAM policy、SCP 與資源權限的影響 | 評估 | +1 | 直接扣回目標 | 判斷 legacy parsing 與 Organizations SCP 的不同層級限制 |
| 驗證公司帳戶前置條件並追蹤第 1 至第 4 章部署狀態 | 驗證 | +2 | 直接扣回目標 | 公司帳戶 CLI 檢查與 Console 操作回報；資源狀態仍待權限允許後核對 |
| 建立 GitHub／Notion／日誌的主管閱讀與積分追蹤機制 | 報告 | +3 | 間接支援 | GitHub private 儀表板、Notion 欄位與本日正式工作日誌；屬 PM/報告支援 |

今日總分：8 分。2026-07-20 依使用者要求改採硬審核口徑重算；未經公司帳戶端到端驗證的 Console 與文件進度只給低分。
...
- 7/15 積分於 2026-07-20 再次改採硬審核標準：未經獨立回驗的 Console 部署操作與 PM 機制建置只算低分，不列為高分里程碑。
