...
|---|---|---:|---|---|
| CloudFormation 版公司帳戶流程成功完成 S1 掃描，並保留真實 RSS run 的掃描輸出 | 掃描 | +4 | 直接扣回目標 | `company-cfn-001` S1 `kept_count=27`；下載報告使用真實 RSS 來源 |
| S2 比較、Quote gate 與 Decision Layer 串接，讓 Top 3 不只依原始平均分，也可看可解釋決策分數 | 比較 | +5 | 直接扣回目標 | S2 `kept_count=6`、Quote approve、`decision-layer.json` 產出 |
| 新增 Evidence Ledger、Human Review Loop、Algorithmic Decision Layer、feedback stats、audit packet 與離線 evaluation harness | 評估 | +8 | 直接扣回目標 | `evaluation_harness.py` quality gate=pass；S5 產出治理與審核 artifacts |
| CloudFormation stack 重建成功，Step Functions `company-cfn-001` 端到端成功，並確認報告可下載檢視 | 驗證 | +9 | 直接扣回目標 | stack resources `CREATE_COMPLETE`；execution `SUCCEEDED`；S3 報告下載驗證 |
| 完成新版報告 artifact、7 頁部會自我介紹簡報、AI 每小時執行軌跡與 7/13-7/16 軌跡補回 | 報告 | +7 | 直接扣回目標 | `report.html`、`outputs/部會自我介紹_王冠婷_AI_PM_7頁版.pptx`、`ai-execution-trace/daily/` |
