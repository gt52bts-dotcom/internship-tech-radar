...
WORKSPACES_AI_AGENT_ACCESS = RecipeDefinition(
    recipe_id="workspaces_ai_agent_access_cdk",
    display_name="WorkSpaces Applications agent access infrastructure",
    display_name_zh="WorkSpaces Applications AI agent access \u57fa\u790e\u8a2d\u65bd\u9a57\u8b49",
    supported_candidate_patterns=(
        "workspaces",
    ...
    required_aws_services=("WorkSpaces Applications", "AppStream", "EC2", "CloudFormation"),
    required_region_capabilities=(
        "workspaces_applications_available",
        "workspaces_agent_access_available",
    ),
    estimated_cost_model_id="workspaces_ai_agent_access",
    deployable_resource_types=(
        "AWS::CDK::Metadata",
        "AWS::AppStream::Fleet",
        "AWS::AppStream::Stack",
    ...
    deployment_inputs_schema={
        "region": {"type": "string", "required": True},
        "stack_name": {"type": "string", "required": True, "note": "Derived from run_id."},
        "fleet_type": {"type": "string", "default": "ON_DEMAND"},
        "instance_type": {"type": "string", "default": "stream.standard.medium"},
        "image_name": {"type": "string", "default": "Amazon-AppStream2-Sample-Image-06-17-2024"},
        "streaming_url_validity_seconds": {"type": "number", "default": 600},
    },
    success_criteria=(
    ...
    evidence_to_collect=(
        ...
        "AppStream describe-stacks AgentAccessConfig fields.",
        "AppStream describe-fleets state and image/instance configuration.",
        "CreateStreamingURL response metadata with the URL redacted.",
        "Pre-cleanup usage snapshot and scoped cleanup result.",
    ),
    ...
    risk_level="high",
    stop_conditions=(
        "Do not continue if the AppStream service-linked role is missing and cannot be created in the account.",
    ...
    poc_directory=PROJECT_ROOT / "poc" / "workspaces-ai-agent-access-cdk-poc",
    draft_notes=(
        "This deployable recipe proves the WorkSpaces Applications agent-access entry point, not a full desktop automation business process.",
        "A later stronger recipe can add a real agent framework, Bedrock model invocation, screenshot inspection, and application task assertions.",
    ),
)

# Compatibility alias for older imports/tests. The recipe is no longer a draft.
WORKSPACES_AI_AGENT_ACCESS_DRAFT = WORKSPACES_AI_AGENT_ACCESS
