...
class CostQuoteTests(unittest.TestCase):
    ...
    def test_workspaces_ai_agent_quote_is_level_a_and_itemized(self):
        ...
        self.assertEqual(quote["validation_scope"], "phase1_infrastructure_only_no_streaming_session")
        self.assertEqual(quote["expected_total_usd"], 0.1)
        self.assertEqual(quote["estimated_range_usd"]["low"], 0.05)
        self.assertEqual(quote["estimated_range_usd"]["high"], 0.4)
        self.assertEqual(quote["recommended_approval_ceiling_usd"], 0.5)
        self.assertEqual(quote["deferred_full_session_estimate"]["expected_total_usd"], 6.5325)
        self.assertEqual(quote["deferred_full_session_estimate"]["recommended_minimum_ceiling_usd"], 14.0)
        self.assertIn("cleanup_cannot_refund", quote["cost_containment_model"])
