...
8. Only when the output status is `cleanup_verified`, invoke Skill 5 with `--runtime .\out\run\s4-runtime-cleaned.json`. New v3 runtime needs screenshot metadata to become final; abort cleanup is reported as incomplete for actual-PoC final purposes.

## Timeout / Abort

If the reviewer cannot confirm within the packet timeout or the normal close path fails, prioritize cost control while preserving the record:
...
- Exception: use `s4-abort --execute` only for timeout, failed deployment, or failed close risk; record the named approver and reason.
